# main-sim-mixed-scale-high.R: script to run a set of simulations to assess identifiability
...
#                  this as well). 
n.time <- 10
# Two replicates at 10 percent of sites
n.rep <- matrix(NA, J, max(n.time))
for (t in 1:n.time) {
  n.rep[, t] <- ifelse(rbinom(J, 1, 0.1) == 1, 2, 1)
}
...
save(psi.mean.samples, psi.low.samples, psi.high.samples,
     ...
     psi.true, beta, scenario.vals, file = 'results/sim-mixed-scale-high-results.rda')
