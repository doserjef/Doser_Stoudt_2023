simTOccDesign <- function(J.x, J.y, n.time, n.rep, n.rep.max, beta, alpha, sp.only = 0, 
		          ...
		          scale.param = 1, sample.design = 'random', ...) {
  ...
  # Sampling design -------------------
  if (!(sample.design %in% c('random', 'systematic', 'cluster'))) {
    stop("sampling.design must be either 'random', 'systematic', or 'cluster'")
  }
  ...
  # Systematic ------------------------
  if (sample.design == 'systematic') {
    s.x <- seq(0, 1, length.out = J.x)
    s.y <- seq(0, 1, length.out = J.y)
    coords <- as.matrix(expand.grid(s.x, s.y))
  } else if (sample.design == 'random') {
    coords <- matrix(runif(J * 2, 0, 1), J, 2)
  } else if (sample.design == 'cluster') {
    # TODO: hardcoded and not done yet. 
    cluster.size <- 4
    s.x <- seq(0, 1, length.out = J.x / 2)
    s.y <- seq(0, 1, length.out = J.y / 2)
    centers <- as.matrix(expand.grid(s.x, s.y))
  }
  if (sp) {
