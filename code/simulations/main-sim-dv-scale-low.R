# main-sim-dv-scale-low.R: script to run a set of simulations to assess identifiability
#                          of a double-visit multi-season occupancy model under varying
#                          levels of spatial and temporal autocorrelation. This generates
#                          data that are misspecified with a scale parameter of 0.5. 
...
for (j in 1:n.sims) {
  ...
  for (i in 1:n.scenarios) {
    ...
    # Site x Replicate
    y <- dat$y[, , 1:n.rep, drop = FALSE]
    # Occurrence Covariates
    X <- dat$X
    # Detection Covariates
    X.p <- dat$X.p[, , 1:n.rep, , drop = FALSE]
    # Coordinates
    coords <- dat$coords
...
save(psi.mean.samples, psi.low.samples, psi.high.samples,
     ...
     psi.true, beta, scenario.vals, file = 'results/sim-dv-scale-low-results.rda')
