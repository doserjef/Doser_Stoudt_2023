# get-sim-data-scale-low.R: script to generate the data sets with scaled logisitic
#                            mis-specification (with a low scale parameter)
...
for (j in 1:n.sims) {
  ...
  for (i in 1:n.scenarios) {
    ...
    dat[[(j - 1) * n.scenarios + i]] <- simTOcc(J.x = J.x, J.y = J.y, n.time = rep(n.time, J), 
		   ...
		   mis.spec.type = 'scale', scale.param = 0.5)
  }
}
save(dat, file = 'data/sim-data-scale-low.rda')
