# get-sim-data-scale-high.R: script to generate the data sets with scaled logisitic
#                            mis-specification (with a high scale parameter)
...
for (j in 1:n.sims) {
  ...
  for (i in 1:n.scenarios) {
    ...
    dat[[(j - 1) * n.scenarios + i]] <- simTOcc(J.x = J.x, J.y = J.y, n.time = rep(n.time, J), 
		   ...
		   mis.spec.type = 'scale', scale.param = 0.8)
  }
}
save(dat, file = 'data/sim-data-scale-high.rda')
