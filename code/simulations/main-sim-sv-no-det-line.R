# main-sim-sv-no-det-line.R: script to run a set of simulations to assess identifiability
#                            of occupancy probability using a logistic regression
#                            model that ignores imperfect detection across multiple
...
for (j in 1:n.sims) {
  ...
  for (i in 1:n.scenarios) {
    ...
    # Coordinates
    coords <- dat$coords
    # Binomial weights for the svcTPGBinom function in spOccupancy
    weights <- matrix(1, J, n.time)
    # Package all data into a list (ignoring detection probability)
    covs <- list(int = X[, , 1],
                 occ.cov.1 = X[, , 2])
    data.list <- list(y = y, covs = covs, weights = weights, coords = coords)
    # Priors
    ...
    # Fit the model with svcTPGBinom
    out <- svcTPGBinom(formula = ~ occ.cov.1,
		       data = data.list,
		       n.batch = n.batch,
		       ...
		       accept.rate = 0.43,
		       cov.model = cov.model,
		       svc.cols = 1, # Space-varying intercept only
		       tuning = tuning.list,
		       n.omp.threads = 1, 
		       verbose = TRUE,
		       ar1 = TRUE,
...
save(psi.mean.samples, psi.low.samples, psi.high.samples,
     ...
     psi.true, beta, scenario.vals, file = 'results/sim-sv-no-det-line-results.rda')
