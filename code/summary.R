...
coverage.df <- data.frame(logit = rep(NA, 4),
			  probit = rep(NA, 4),
			  scale.high = rep(NA, 4),
			  scale.low = rep(NA, 4),
			  line = rep(NA, 4))
...
avg.df.full$type <- factor(avg.df.full$type, levels = c('Logit', 'Probit', 
...
avg.df.2.full$type <- factor(avg.df.2.full$type, levels = c('Logit', 'Probit', 
							'Scale High', 'Scale Low', 
							'Linear'), 
			   ordered = TRUE)
...
# 	    diff.quant.high = quantile(abs(diff), 0.975))
# avg.df.scale.low$type <- 'Scale Low'
# save(avg.df.scale.low, file = 'results/dv-scale-low-bias-df.rda')
