# main-sim-five-visits-scale-high.R: script to run a set of simulations to assess identifiability
...
rm(list = ls())
library(spOccupancy)
# Load the hacked simTOcc function
  ...
  for (i in 1:n.scenarios) {
    ...
    dat <- simTOcc(J.x = J.x, J.y = J.y, n.time = rep(n.time, J), 
		   ...
		   ar1 = ar1, rho = rho.curr, sigma.sq.t = sigma.sq.t.curr, 
                   scale.param = 0.8, mis.spec.type = 'scale')
...
save(psi.mean.samples, psi.low.samples, psi.high.samples,
     ...
     psi.true, beta, scenario.vals, file = 'results/sim-five-visits-scale-high-results.rda')
